...
#ifndef UGPLUGIN_XBRAIDFORUG4_INTEGRATOR_THETA_RESIDUAL_INTEGRATOR_NL_HPP
#define UGPLUGIN_XBRAIDFORUG4_INTEGRATOR_THETA_RESIDUAL_INTEGRATOR_NL_HPP
...
namespace ug{ namespace xbraid {
    ...
    class ThetaResidualIntegratorNL : public IResidualTimeIntegrator<TDomain, TAlgebra> {
    public:
        //--------------------------------------------------------------------------------------------------------------
        ...
        ThetaResidualIntegratorNL() : IResidualTimeIntegrator<TDomain, TAlgebra>() {}
        ~ThetaResidualIntegratorNL() override = default;
        ...
        SP_GridFunction defect(SP_GridFunction u1, number t1, CP_GridFunction u0, number t0) override {
            ...
            // auto result = m_non_linear_solver->prepare(*u1_clone.get());
            // std::cout << "prepare " << result << std::endl << std::flush;
            // todo (ð) non_linear_solver_->defect(*rhs,*u1);
            return rhs;
        };
        //--------------------------------------------------------------------------------------------------------------
