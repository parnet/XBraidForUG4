//todo check
#ifndef UG_PLUGIN_XBRAIDFORUG4_BRAID_EXECUTOR_H
#define UG_PLUGIN_XBRAIDFORUG4_BRAID_EXECUTOR_H

#include "bindings/lua/lua_user_data.h"
#include "../driver/basic_driver.h"
#include "../interface/observer_xbraid.h"
#include "space_time_communicator.h"
#include "braid_settings.h"
...
namespace ug { namespace XBraidForUG4 {
    ...
    public:
        ...
        typedef BraidGridFunctionBase<TDomain, TAlgebra> T_BraidBaseApp;
        typedef SmartPtr<T_BraidBaseApp> SP_BraidBaseApp;

        typedef DomainDiscretization<TDomain, TAlgebra> T_PDomainDisc;
        typedef SmartPtr<T_PDomainDisc> SP_DomainDisc; // todo delete ? - not used
        ...
        typedef SmartPtr<T_ITimeIntegratorObserver> SP_T_ITimeIntegratorObserver;

        typedef IXBraidTimeIntegratorObserver<TDomain, TAlgebra> T_IXBraidTimeIntegratorObserver;
        typedef SmartPtr<T_IXBraidTimeIntegratorObserver> SP_XBTIObserver;

        typedef BraidInitializer<TDomain, TAlgebra> T_BraidInitializer;
        typedef SmartPtr<T_BraidInitializer> SP_BraidInitializer;

        typedef BraidSettings T_BraidSettings;
        typedef SmartPtr<T_BraidSettings> SP_BraidSettings;
        ...
        void set_residual(bool residual) {
            if (residual && this->m_app->can_residual_method) {
        ...
        void reset_core() {
            delete m_braid_core;
            m_braid_core = new BraidCore(this->m_comm->GLOBAL, this->m_app.get()); // todo make smartptr?
        }
        ...
        SP_BraidBaseApp get_app() {
            return this->m_app;
        }

        void set_settings(SP_BraidSettings settings) { }// todo implement or delete


        SP_BraidSettings get_settings() {}// todo implement or delete

        void apply_settings(T_BraidSettings settings) {}  // todo implement or delete
        ...
        void set_observer(SP_T_ITimeIntegratorObserver output) {
            this->m_app->set_observer(output);
        }

        void set_xbraid_observer(SP_XBTIObserver output) {
            this->m_app->set_xbraid_observer(output);
        }
        ...
        bool apply(SP_GridFunction u_stop, number t_stop, SP_GridFunction u0, number t0) {
            ...
            if (this->m_app->m_initializer == SPNULL) {
                this->m_app->m_initializer = make_sp(new GridFunctionInitializer<TDomain,TAlgebra>());
            }
            this->m_app->m_initializer->set_start_values(u0, t0);

            this->m_app->restimate = this->m_braid_settings.m_est_error; // todo why here?
            this->m_app->refine_time = this->m_braid_settings.m_refine; // todo why here?
        ...
        void set_parallel_logger(SP_ParallelLogger log) {
            this->m_app->set_parallel_logger(log);
        }
        //--------------------------------------------------------------------------------------------------------------
    };
}}
...
#endif //UG_PLUGIN_XBRAIDFORUG4_BRAID_EXECUTOR_H
